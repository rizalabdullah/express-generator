  ...
  try{
    ...
    if(req.file){
      ...
      //  let filename = req.file.filename + '.' + originalExt;
       ...
       let target_path = path.resolve(config.rootPath, `public/images/products/${filename1}`);
       ...
       src.on('end', async () => {
        try {
          ...
          let product = new Product({...payload, image_url: filename1})
          await product.save()
      ...
      let filename1 = req.file.originalname;
       ...
       src.on('end', async () => {
        try {
          ...
          product = await Product.findByIdAndUpdate(id, {...payload, image_url: filename1}, {
            new: true,
            runValidators: true
          });
          return res.json(product);
